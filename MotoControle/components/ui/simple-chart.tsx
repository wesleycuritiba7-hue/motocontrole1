...
export function SimpleBarChart({
  ...
  const maxVal = Math.max(...data, ...(secondaryData || [0]), 1);
    ...
    <View style={{ height: height + 20 }}>
      ...
      <View className="flex-row items-end justify-between flex-1 px-1">
        {data.map((val, i) => {
          ...
          const secH = secondaryData ? (secondaryData[i] / maxVal) * (height - 40) : 0;
...
// ==================== GROUPED BAR CHART (Barras agrupadas por categoria) ====================
...
export function SimpleLineChart({
  ...
  return (
    <View className="items-center">
      {showTotal && total > 0 && (
        <View className="flex-row justify-end w-full mb-1 pr-2">
          <Text className="text-xs font-bold" style={{ color: themeColors.success }}>
            Total: {formatCurrency(datasets.reduce((s, ds) => s + ds.data.reduce((a, b) => a + b, 0), 0))}
          </Text>
        </View>
      )}
      <Svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
